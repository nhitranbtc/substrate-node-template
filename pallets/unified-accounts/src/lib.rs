...
//! * `claim_default_evm_address`: Creates the double mapping with default evm address given that no
//!   prior mapping exists.
//!
//! ## Storage Fee
//! User is also charged a storage fee
//! [`AccountMappingStorageFee`](`crate::Config::AccountMappingStorageFee`) before mappings are
//! created to prevent storage abuse.
//!
//! WARNINGS:
//! * This pallet only handles transfer of native balance only, for the rest of native assets hold
//!   by evm address like XC20, DAppStaking unclaimed rewards, etc should be transferred manually
//!   beforehand by user himself otherwise FUNDS WILL BE LOST FOREVER.
...
//! * [`AddressMapping`](pallet_evm::AddressMapping): Wrapper over `UnifiedAddressMapper` for evm
//!   address mapping to account id.
//! * `KillAccountMapping`: [`OnKilledAccount`](frame_support::traits::OnKilledAccount)
//!   implementation to remove the mappings from storage after account is reaped.
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		/// The amount of currency needed for mapping to be added.
		/// Two storage items with values sizes, sizeof(AccountId) and sizeof(H160) respectively.
		#[pallet::constant]
	...
	pub enum Event<T: Config> {
		...
		AccountClaimed { account_id: T::AccountId, evm_address: EvmAddress },
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn claim_evm_address(
			...
			ensure!(!NativeToEvm::<T>::contains_key(&who), Error::<T>::AlreadyMapped);

			ensure!(!EvmToNative::<T>::contains_key(evm_address), Error::<T>::AlreadyMapped);
			...
			if frame_system::Pallet::<T>::account_exists(&default_account_id) {
				/// Transfer all the free native balance old account id to the newly
			...
			Self::deposit_event(Event::AccountClaimed { account_id: who, evm_address });
...
impl<T: Config> Pallet<T> {
	...
	fn do_claim_default_evm_address(account_id: T::AccountId) -> Result<EvmAddress, DispatchError> {
		ensure!(!NativeToEvm::<T>::contains_key(&account_id), Error::<T>::AlreadyMapped);

		// get the default evm address
		let evm_address = T::DefaultMappings::to_default_h160(&account_id);
		// make sure default address is not already mapped, this should not happen but for sanity
		// check.
		...
		// create double mapping for the pair with default evm address
		EvmToNative::<T>::insert(&evm_address, &account_id);
		NativeToEvm::<T>::insert(&account_id, &evm_address);

		Self::deposit_event(Event::AccountClaimed { account_id, evm_address });
		Ok(evm_address)
	}
	...
	fn charge_storage_fee(who: &T::AccountId) -> Result<Balance, DispatchError> {
		...
		T::Currency::burn_from(who, T::AccountMappingStorageFee::get(), Preserve, Exact, Polite)
	}
}
	...
	fn build_domain_separator() -> [u8; 32] {
		let mut domain =
			keccak256!("EIP712Domain(string name,string version,uint256 chainId,bytes32 salt)")
				.to_vec();
		domain.extend_from_slice(&keccak256!("Substrate node template node")); // name
	...
	fn lookup(a: Self::Source) -> Result<Self::Target, LookupError> {
		match a {
			MultiAddress::Address20(i) =>
				Ok(<Self as UnifiedAddressMapper<T::AccountId>>::to_account_id_or_default(
