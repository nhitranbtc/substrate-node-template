...
///
/// When an asset is created, it automatically becomes available to the EVM via an `ERC20-like`
/// interface. In order for the precompile to work, dedicated asset address needs to have the revert
/// code registered, otherwise the call will fail.
///
/// It is important to note that if the dedicated asset EVM address is already taken, asset creation
/// should fail. After asset has been destroyed, it is also safe to remove the revert code and free
/// the address for future usage.
...
where
	...
	R: pallet_evm::Config,
{
	fn created(id: &AssetId, _owner: &AccountId) -> Result<(), ()> {
...
/// Mapping derieved from hashing the original address
...
impl<H: Hasher<Out = H256>> UnifiedAddressMapper<AccountId> for HashedDefaultMappings<H> {
	...
	fn to_account_id(_evm_address: &EvmAddress) -> Option<AccountId> {
		None
	}

	fn to_h160(_account_id: &AccountId) -> Option<EvmAddress> {
		None
	}
}
...
pub enum UnifiedAddress<Address> {
	/// The address fetched from the mappings and the account is unified.
	#[codec(index = 0)]
	Mapped(Address),

	/// The default address associated with account as there is no mapping found and accounts are
	/// not unified.
	#[codec(index = 1)]
	Default(Address),
}
